...
import { useApp } from '@/contexts/AppContext';
import { useRouter } from 'expo-router';
import { Search, Chrome as Home, MapPin, Calendar, User, ArrowRight, CircleAlert as AlertCircle, Phone, Hash, Globe } from 'lucide-react-native';
...
export default function SearchScreen() {
  ...
  const handleSearch = async () => {
    if (!searchValue.trim()) {
      setError(searchType === 'application' ? t('please_enter_application') : t('please_enter_mobile'));
      return;
    }
    ...
    try {
      ...
      if (flat) {
        ...
        setError(searchType === 'application' ? t('no_flat_found_app') : t('no_flat_found_mobile'));
      }
    } catch (err) {
      setError('Failed to search. Please try again.');
    } finally {
